 ...
 * @source https://raw.githubusercontent.com/bahadyxd/betternitro-de/main/betternitro.plugin.js
 * @updateUrl https://raw.githubusercontent.com/bahadyxd/betternitro-de/main/betternitro.plugin.js
 */
/*@cc_on
@if (@_jscript)
                ...
                getSettingsPanel() {
                    return Settings.SettingPanel.build(_ => this.saveAndUpdate(), ...[
                        new Settings.SettingGroup("Features").append(...[
                            new Settings.Switch("Hochwertige Bildschirmfreigabe", "Aktivieren oder deaktivieren Sie 1080p/Quelle @ 60fps Bildschirmfreigabe ", this.settings.screenSharing, value => this.settings.screenSharing = value)
                        ]),
                        new Settings.SettingGroup("Emoji").append(
                            new Settings.Switch("Verwenden von animierten / stillen Emoji", "Öffne No Nitro-Emojis", this.settings.emojiBypass, value => this.settings.emojiBypass = value),
                            new Settings.Slider("Größe", "Emoji-Pixelgröße. 40 Empfohlen", 16, 64, this.settings.emojiSize, size=>this.settings.emojiSize = size, {markers:[16,20,32,40,64], stickToMarkers:true})
                        ),
                            new Settings.SettingGroup("Profilfoto").append(...[
                                new Settings.Switch("Profilfoto", "Client-unterstützte Profilbilder aktivieren oder deaktivieren.", this.settings.clientsidePfp, value => this.settings.clientsidePfp = value),
                                new Settings.Textbox("URL", "Profile Photo Link.", this.settings.pfpUrl,
                                    image => {
                                        try {
                                            new URL(image)
                                        } catch {
                                            return Toasts.error('Falscher Link!')
                                        }
